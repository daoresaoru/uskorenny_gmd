...
MAX_LINE_PX   = 612   # pixels — Palatino Linotype 16pt, fits sample line
...
# Shared font metrics for line-length measurement (Palatino Linotype 16pt)
_MEASURE_FONT = QFont("Palatino Linotype", 16)
...
class DialoguePreview(QWidget):
    ...
    def start_animation(self, full_text: str, on_finish=None):
        self._full_text = full_text
        self._anim_pos = 0
        self._on_finish = on_finish
        self._timer = QTimer()
        self._timer.setInterval(50)  # 100ms = 10 chars/sec
        self._timer.timeout.connect(self._anim_tick)
        self._timer.start()
        
    def start_animation_scheduled(self, schedule: list[tuple[str, int, int]], on_finish=None):
        self._full_text = "".join(t for t, _, __ in schedule)
        ...
        self._timer.setInterval(schedule[0][2] if schedule else 100)
    ...
    def _scheduled_tick(self):
        ...
        chunk_text, pause_ms, interval_ms = self._schedule[self._chunk_idx]
        self._anim_pos += 1

        if self._anim_pos <= len(chunk_text):
            self._text = self._displayed + chunk_text[:self._anim_pos]
            self.update()

        if self._anim_pos >= len(chunk_text):
            self._displayed += chunk_text
            self._chunk_idx += 1
            self._anim_pos = 0
            # Set speed for next chunk
            if self._chunk_idx < len(self._schedule):
                self._timer.setInterval(self._schedule[self._chunk_idx][2])
            if pause_ms > 0:
                self._timer.stop()
                QTimer.singleShot(pause_ms, self._resume_after_pause)
    ...
    def stop_animation(self):
        if hasattr(self, '_timer') and self._timer.isActive():
            self._timer.stop()
        self._text = getattr(self, '_full_text', self._text)
        self.update()

    def _anim_tick(self):
        self._anim_pos += 1
        self._text = self._full_text[:self._anim_pos]
        self.update()
        if self._anim_pos >= len(self._full_text):
            self._timer.stop()
            if self._on_finish:
                self._on_finish()
...
class SegmentRow(QFrame):
    ...
    def _toggle_animation(self):
        if self._animating:
            ...
            self.page_preview.stop_animation()
            self._update_page_preview()
        else:
            # Build list of (text, pause_ms) per chunk
            chunks = parse_chunks(self.raw_segment)
            translations = self.get_translations()
            schedule = []
            for i, chunk in enumerate(chunks):
                text = re.sub(r'<[^>]+>', '', translations[i]) if i < len(translations) else ""
                # Pause from <E003 X>
                pause_ms = 0
                m = re.search(r'<E003\s+(\d+)>', chunk["pause_tag"])
                if m:
                    pause_ms = int(m.group(1))
                # Speed from <E025 X> in prefix_tags
                interval_ms = 100  # default
                s = re.search(r'<E025\s+(\d+)>', chunk["prefix_tags"])
                if s:
                    x = int(s.group(1))
                    interval_ms = 25 + (x - 1) * 25
                schedule.append((text, pause_ms, interval_ms))

            combined = "".join(t for t, _, __ in schedule)
            ...
            self.play_btn.setStyleSheet(
                "background: #443333; color: #ffaaaa; font-size: 11px; padding: 2px 6px; border-radius: 3px;"
            )
            self.page_preview.start_animation_scheduled(schedule, on_finish=self._on_anim_finish)
...
class Tab1(QWidget):
    ...
    def inject_translations(self):
        ...
        QMessageBox.information(
            ...
            "Нажмите «Сохранить .json» для сохранения проекта."
        )
...
class Tab2(QWidget):
    ...
    def save_to_file(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Сохранить", "", "Мой формат JSON (*.json)"
        )
        if not path:
            return
        data = {
